...
export async function getPlaylistTracks(accessToken, playlistId) {
  ...
  // Handle Spotify's undocumented property naming
  const tracksData = data.tracks ?? data.items ?? {};
  let items = tracksData.items || [];
  let nextUrl = tracksData.next;

  // Fetch subsequent pages of tracks (up to 500 total tracks)
  while (nextUrl && items.length < 500) {
    try {
      ...
      if (nextRes.ok) {
        ...
        console.error(`Failed to fetch next tracks page: ${nextRes.status}`);
        break;
      }
    } catch (e) {
      console.error('Error fetching next tracks page:', e);
      break;
    }
  }
  ...
  return items
    .map((item) => {
      // Spotify sometimes nests the track data under 'item' instead of 'track',
      // and sets 'track' to a boolean true.
      const trackData = (typeof item.track === 'object' && item.track) ? item.track : item.item;
      return trackData;
    })
    ...
    .map((track) => ({
      ...
      previewUrl: track.preview_url || null, // Will likely be null now, iTunes fallback handles this
