      ...
      if (trackCount === 0 && p.id) {
        try {
          const plRes = await fetch(
            `${SPOTIFY_API_BASE}/playlists/${p.id}?fields=tracks(total)`,
            { headers: { Authorization: `Bearer ${accessToken}` } }
          );
          if (plRes.ok) {
            const plData = await plRes.json();
            trackCount = plData.tracks?.total ?? 0;
          }
        } catch {
...
export async function getPlaylistTracks(accessToken, playlistId) {
  const items = await fetchAllPages(
    `${SPOTIFY_API_BASE}/playlists/${playlistId}/tracks?limit=100&fields=items(track(id,name,artists,album(name,images),preview_url,duration_ms)),next`,
    accessToken,
    2000
  );
...
export async function getPlaylistInfo(accessToken, playlistId) {
  const res = await fetch(
    `${SPOTIFY_API_BASE}/playlists/${playlistId}?fields=id,name,description,images,tracks(total)`,
    {
      headers: { Authorization: `Bearer ${accessToken}` },
    }
  );
